	...
	public List<float[]> list3D{
		get{ 
			return new List<float []>(){
				...
				new float[]{-40,-40f, 100f},
				new float[]{-40,-0f, 100f},
				new float[]{40,-0f, 100f},
				new float[]{-25,-25f, 100f},
				new float[]{-40f,40f, 100f},
				new float[]{0f,40f, 100f},
				new float[]{0f,-40f, 100f},
...
//	public Texture2D circleTexture;
	...
	public void CreateEye0ImageMaterial ()
	{
		if (!FramePublishingVariables.eye0ImageMaterial)
		{
		    ...
		    FramePublishingVariables.eye0ImageMaterial.hideFlags = HideFlags.None;
		}
	}
	public void CreateEye1ImageMaterial ()
	{
		if (!FramePublishingVariables.eye1ImageMaterial)
		{
			...
			FramePublishingVariables.eye1ImageMaterial.hideFlags = HideFlags.None;
		}
	}
	static void CreateEyeSphereMaterial ()
	{
		if (!eyeSphereMaterial)
		{
			...
			eyeSphereMaterial.hideFlags = HideFlags.None;
			// Turn on alpha blending
...
//			newStr = json.Remove (0, json.IndexOf ("\""));
	...
	long lastTick;
	float elapsedTime;
    private bool shouldShowDot = true;
    ...
    public void SetShowDot(bool shouldShow)
    {
        shouldShowDot = shouldShow;
    }
	...
	void OnGazePacket(){
		...
		if (m_status == EStatus.ProcessingGaze) {
			...
			if (CurrentCalibrationMode == CalibModes._3D) {
			    ...
			    gaze3D.shape.x = (shouldShowDot) ? -Pupil.values.GazePoint3D.x : -1000000;
			    gaze3D.shape.y = (shouldShowDot) ? -Pupil.values.GazePoint3D.y : -1000000;
			    gaze3D.depth = (shouldShowDot) ? Pupil.values.GazePoint3D.z : -1000000;
			}
